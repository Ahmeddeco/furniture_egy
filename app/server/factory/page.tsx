...
import { getAllFactory } from "@/database/factory"
import { deleteFactory } from "@/actions/factoryAction"
...
export default async function FactoryPage({ searchParams }: { searchParams: Promise<{ page: string; size: string }> }) {
	...
	return (
		...
		>
			{!factories ? (
				<EmptyCard href={"/server/factory/add"} linkTitle={"add factory"} linkIcon={Factory} />
			) : (
				<Table>
					{/* ---------------------------- TableHeader ---------------------------- */}
					<TableHeader>
										...
										<DropdownMenuContent align="end" className="space-y-2">
											<DropdownMenuItem asChild>
												<Button variant={"default"} size={"full"} asChild>
													<Link href={`/server/factory/edit/${id}`}>edit</Link>
												</Button>
											</DropdownMenuItem>
											{/* ---------------------------- delete --------------------------- */}
											<DropdownMenuItem asChild>
